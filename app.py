...
def init_db():
    if not os.path.exists(DATA_FILE):
        df = pd.DataFrame(columns=[
            ...
            "處理人", "狀態", "問題描述", "截圖_Base64", "百昌回覆", "百昌截圖_Base64", "討論次數", "延續自ID"
        ])
...
# 計算狀態數量 (用於頁籤與側邊欄)
active_count = len(df[df["狀態"].isin(["已提報", "處理中", "討論重啟"])])
    ...
    with st.expander("⚙️ 系統備份與還原管理", expanded=False):
        ...
        if admin_unlocked:
            ...
            if st.button("🧨 確定清空全庫資料 (歸零)", type="primary", use_container_width=True):
                empty_df = pd.DataFrame(columns=["Issue_ID", "建立日期", "最後更新", "模組", "優先級", "處理人", "狀態", "問題描述", "截圖_Base64", "百昌回覆", "百昌截圖_Base64", "討論次數", "延續自ID"])
    ...
    with st.form("new_issue", clear_on_submit=True):
        ...
        if st.form_submit_button("📢 提交問題"):
            if not desc.strip(): st.error("請輸入詳細問題描述！")
                ...
                new_row = {"Issue_ID": new_id, "建立日期": datetime.now().strftime("%Y-%m-%d"), "最後更新": datetime.now().strftime("%Y-%m-%d"), "模組": module, "優先級": priority, "處理人": assignee, "狀態": "已提報", "問題描述": desc.strip(), "截圖_Base64": imgs_to_base64(imgs, "初始提報"), "百昌回覆": "", "百昌截圖_Base64": "", "討論次數": "0", "延續自ID": link_id}
        ...
        if rid:
            ...
            if btn_reject:
                if not reason.strip(): st.error("⚠️ 請填寫問題，讓百昌了解問題方向！")
                    ...
                    df = load_data()
                    idx = df[df["Issue_ID"] == rid].index[0]
                    rt = int(df.at[idx, "討論次數"]) if str(df.at[idx, "討論次數"]).isdigit() else 0
                    df.at[idx, "狀態"], df.at[idx, "討論次數"], df.at[idx, "最後更新"] = "討論重啟", str(rt + 1), datetime.now().strftime("%Y-%m-%d %H:%M")
...
with tab4:
    ...
    if search_id:
        ...
        st.write(f"**狀態:** {r['狀態']} | **重新討論:** {r['討論次數']} 次 | **建立日期:** {r['建立日期']}")
...
with tab5:
    if not df.empty:
        ...
        k2.metric("討論次數", int(pd.to_numeric(df['討論次數'], errors='coerce').sum()), help="重新討論的總次數")
        ...
        with c2:
            ...
            df['討論次數_數值'] = pd.to_numeric(df['討論次數'], errors='coerce').fillna(0)
            df_rework = df[df['討論次數_數值'] > 0].sort_values(by='討論次數_數值', ascending=False).head(5)
            if not df_rework.empty:
                st.dataframe(df_rework[["Issue_ID", "處理人", "討論次數_數值", "狀態"]], use_container_width=True, hide_index=True)
            else:
